 ...
 *
 * Validation:
 *  - Reads OLD MagnetOffset from FLASH (old zero), computes NEW = -absNow (mod 1), persists it,
 *    reads back to verify within 1 LSB, and then checks absolute angles post-write are ~0 within 2 LSBs.
 *  - Skips entirely in desktop simulation (no real CAN / no FLASH).
 */
...
private static StatusCode applyOffsetPersist(CANcoder enc, double offsetTurns) {
  ...
  if (sc.isError()) {
    ...
    System.out.printf("[SwerveCal][WARN] FLASH write status: %s (expected during FLASH commit)%n", name);
  } else {
  ...
  private static void logModule(
      String name, double absNow, double oldOff, double newOff, double readBack, StatusCode st) {
  ...
  private static void verifyWritten(String name, double requested, double readBack) {
    ...
    if (diff > kWriteVerifyEps) {
      ...
      System.out.printf("[SwerveCal] %s persisted OK (|diff|=%.6f ≤ %.6f ~ 1 LSB).%n",
